...
export default function Navbar() {
  ...
  const navLinks = [
    { to: '/',           label: t('home'),       icon: FiHome,     public: true },
    { to: '/search',     label: t('findDonors'), icon: FiSearch,   public: true },
    { to: '/bloodbanks', label: t('bloodBanks'), icon: FiHeart,    public: true },
    { to: '/dashboard',  label: t('dashboard'),  icon: FiActivity, auth: true },
    { to: '/admin',      label: t('admin'),      icon: FiShield,   role: 'ADMIN' },
  ]
    ...
    <nav className="sticky top-0 z-50 bg-gray-950/80 backdrop-blur-xl border-b border-white/5">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
                ...
                <Link
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium
                    ...
                    ${active
                      ...
                      : 'text-gray-400 dark:text-gray-500 transition-colors hover:text-gray-100 hover:bg-white/5'
                    }`}
                >
            ...
            {isLoggedIn ? (
              <div className="flex items-center gap-3">
                ...
                <button onClick={handleLogout}
                  className="flex items-center gap-2 px-3 py-2 text-sm text-gray-400 dark:text-gray-500 transition-colors
              ...
              <div className="flex items-center gap-2">
                ...
                <Link to="/add-donor" className="btn-primary text-sm px-4 py-2">
                  {t('donorRegister')}
                </Link>
              </div>
            )}
          ...
          {/* Mobile hamburger */}
          <button
            className="md:hidden p-2 text-gray-400 dark:text-gray-500 transition-colors hover:text-white transition-colors"
            ...
            <div className="pt-3 border-t border-white/5">
              {isLoggedIn ? (
                ...
                <div className="space-y-2">
                  ...
                  <Link to="/add-donor" onClick={() => setOpen(false)} className="btn-primary w-full justify-center text-sm">
                    Donor Register
                  </Link>
                </div>
              )}
